...
public class ControllerLibroMayor implements Initializable {
    ...
    public ObservableList<String> getCuentasList(){
        ...
        String query = "SELECT nombre FROM cuentas WHERE recibe_saldo='Si';";
        Statement st;
        ResultSet rs;
        try {
    ...
    public ObservableList<AsientoCuenta> getAsientoCuentaList(){
        ...
        Connection connection = getConnection();
        String query = "SELECT nombre,fecha,descripcion,debe,haber,saldo FROM asientos a INNER JOIN cuenta_asiento ca on(a.id=ca.idasiento) INNER JOIN cuentas c on (ca.idcuenta=c.codigo) WHERE c.nombre='"+ choiceBox.getValue() + "' and fecha>= '"+fechaInicial.getValue()+"' and fecha<= '"+fechaFinal.getValue()+"' ORDER BY fecha,idasiento;";
        Statement st;
        ResultSet rs;
        try {
            st = connection.createStatement();
    ...
    //Muestra la consulta realizada en la tabla
    public void showAsientos() {
        ObservableList<AsientoCuenta> asientosList = getAsientoCuentaList();
        ...
        TableView.setItems(asientosList);
    }
    ...
    public void pdfs()
        ...
        try {
            ...
            PdfWriter.getInstance(my_pdf_report, new FileOutputStream("C:\\Users\\Usuario\\Desktop\\LibroMayor "+choiceBox.getValue()+" Pdf.pdf"));
            ...
            while (rs.next()) {
                ...
                my_report_table.addCell(table_cell);
                String manager_id3 = rs.getString("saldo");
            ...
            Paragraph nombre =new Paragraph("Libro Mayor\nCuenta : "+choiceBox.getValue()+"\n",
                    FontFactory.getFont("Century",
                            20,
                            java.awt.Font.BOLD,
                            BaseColor.BLACK));
